...
def _bbox_remap(bbox: numpy.ndarray, src_size: tuple[int, int], tar_size: tuple[int, int], scaleup=False):
    # 因为进行过加灰边，需要还原
...
def _draw_prediction(
    ...
    for obj in prediction:
        x1, y1, x2, y2 = _bbox_remap(obj[0:4], ori_img_shape, reshaped_img_shape).astype(int)
        conf = obj[4]
        cls = int(obj[10])

        img = cv2.rectangle(img, (x1, y1), (x2, y2), (0, 255, 0), 2)
        text = f"{label_names[cls]}:{conf:%}"
        img = cv2.putText(img, text, (x1, y1), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0),
                            2,
                            cv2.LINE_AA)
    return img
...
def detect(config: Config, input_images: list[str]):
    ...
    def get_batch():
        ...
        img_batch = torch.from_numpy(numpy.stack(images))
        img_batch = torch.nn.functional.pad(img_batch, (0, 0, 0, 0, 0, 0 ,0 ,config.batch_size - img_batch.shape[0]), "constant", 0)
        yield img_batch.to(device, non_blocking=True).float() / 255, img_list
    
    save_dir = config.run_path / "result"      
    os.makedirs(save_dir, exist_ok=True)
          
    for batch, img_list in tqdm.tqdm(get_batch(), total=len(input_images) // config.batch_size):
...
if __name__ == '__main__':
    path = "/run/media/yty/盘盘/datasets/DroneTrainDataset/Drone_TrainSet"
