...
from safe.attack import PDGAttack, FSGMAttack
...
def do_preprocess():
    ...
    preprocess.raw_dataset2h5("run/preprocess/pure_drone_train_full.h5", drone_test)
...
def main(config: Config, data_paths: Sequence[str]):
    ...
    for attacker in attackers:
        mlp, _ = search.train_mlp_from_features(
            feature_name_set,
            list(config.detected_result_dataset.ood_features.keys()),
...
if __name__ == '__main__':
    ...
    data_paths = [
        "run/preprocess/pure_drone_train_full.h5",
