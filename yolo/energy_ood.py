...
def main():
    ...
    ood_names = [os.path.splitext(os.path.basename(ood))[0] for ood in args.ood_features]
    print(f'ood datasets: {ood_names}')

    w, b = np.load(args.fc)
    print(f'{w.shape=}, {b.shape=}')

    train_labels = np.loadtxt(args.train_label, dtype=np.uint32)

    recall = 0.95

    print('load features')
    feature_id_train = np.load(args.id_train_feature).squeeze()
    feature_id_val = np.load(args.id_val_feature).squeeze()
    feature_oods = {
        name: np.load(feat).squeeze() for name, feat in zip(ood_names, args.ood_features)
    }
    ...
    score_id = -np.array(
        [(((f - mean) @ prec) * (f - mean)).sum(dim=-1).min().cpu().item()
